...
namespace HT
{
  ...
  bool HelloTriangleApp::checkValidationLayerSupport(){
    ...
    for(const auto& layer : validationLayers){
      if(std::find_if(
        ...
        availableLayers.end(),
        [&l = layer](const VkLayerProperties& v) -> bool { return l == v.layerName;})
        != availableLayers.end()) 
          return false;
    }
  ...
  void HelloTriangleApp::pickPhysicalDevice(){
    ...
    std::cout << "Using " << physicalDeviceProperties.deviceName << std::endl;
  }
  ...
  bool HelloTriangleApp::isDeviceSuitable(VkPhysicalDevice device){
    ...
    const VkBool32 swapChainAdequate = !swapChainSupport.formats.empty() && !swapChainSupport.presentModes.empty();
  ...
  bool HelloTriangleApp::checkDeviceExtensionSupport(VkPhysicalDevice device){
    ...
    for(const auto& extension: deviceExtensions){
        ...
        [&e = extension](const VkExtensionProperties& v) -> bool { return e == v.extensionName;})
        != availableExtensions.end()) 
          return false;
    }
  ...
  VkSurfaceFormatKHR HelloTriangleApp::chooseSwapChainFormat(const std::vector<VkSurfaceFormatKHR>& availableFormats){
